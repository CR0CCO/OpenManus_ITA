@echo off
:: Attiva la codifica UTF-8 per supportare i caratteri nel terminale
chcp 65001 >nul
...
:: 7. Creazione del collegamento sul Desktop tramite PowerShell (punta esplicitamente a cmd.exe)
echo [Desktop] Creazione/aggiornamento del collegamento sul Desktop in corso...
powershell -NoProfile -Command "$W = New-Object -ComObject WScript.Shell; $S = $W.CreateShortcut([System.IO.Path]::Combine([System.Environment]::GetFolderPath('Desktop'), 'OpenManus_ITA.lnk')); $S.TargetPath = 'cmd.exe'; $S.Arguments = '/c ""' + '%~dp0avvia.bat' + '""'; $S.WorkingDirectory = '%~dp0'; $S.Description = 'Avvia OpenManus ITA'; $S.Save()"
